      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-24 space-y-6">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/5 border border-white/5 rounded-full">
            <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Simple Pricing</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
          {plans.map((plan, i) => (
            ...
            >
              ...
              <div className="mb-8">
                <div className={`text-sm font-bold uppercase tracking-widest mb-3 ${plan.highlight ? 'text-black/50' : 'text-white/30'}`}>
                  {plan.name}
                </div>
                <div className="flex items-end gap-1 mb-3">
                  <span className="text-5xl font-bold tracking-tighter">{plan.price}</span>
                  <span className={`text-sm font-medium mb-2 ${plan.highlight ? 'text-black/40' : 'text-white/30'}`}>{plan.period}</span>
                </div>
                <p className={`text-sm leading-relaxed ${plan.highlight ? 'text-black/60' : 'text-white/40'}`}>{plan.description}</p>
              </div>
