...
export default function SuperadminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background flex">
      {/* Sidebar */}
      <aside className="w-72 bg-background border-r border-border flex flex-col fixed inset-y-0 left-0 z-50">
        ...
        <nav className="flex-1 p-4 space-y-1">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`w-5 h-5 ${isActive ? 'text-orange-400' : 'text-foreground/40'}`} />
                {item.label}
              </Link>
            );
          })}
          ...
          <div className="pt-4 border-t border-border mt-4">
            <Link
              ...
              className="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium text-foreground/25 hover:text-foreground/60 hover:bg-white/[0.03] transition-all"
            >
        ...
        <div className="p-4 border-t border-border">
          <div className="flex items-center justify-between mb-4 px-2">
            <div className="flex items-center gap-3 min-w-0 pr-2">
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center text-foreground text-xs font-bold shadow-lg">
                {admin?.name?.charAt(0) || 'S'}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-foreground text-sm font-medium truncate">{admin?.name}</p>
                <p className="text-foreground/25 text-[11px] truncate">{admin?.email}</p>
              </div>
            </div>
            <ThemeToggle />
          </div>
