  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-white/30 mt-1 text-sm">Global statistics across all tenants</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {kpiCards.map((card) => {
          ...
          return (
            <div key={card.label} className="bg-white/[0.02] border border-white/[0.04] rounded-2xl p-5 hover:border-white/[0.08] transition group">
              <div className="flex items-center justify-between mb-4">
                <div className={`w-10 h-10 rounded-xl ${card.bg} flex items-center justify-center`}>
                  <Icon className={`w-5 h-5 ${card.accent}`} />
                </div>
              </div>
              <p className="text-3xl font-bold text-white tracking-tight">{card.value.toLocaleString()}</p>
              <p className="text-white/30 text-xs mt-1 font-medium">{card.label}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="flex items-end gap-3 h-48">
            {chartData.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2">
                <span className="text-[10px] text-white/40 font-bold">{d.leads}</span>
                <div
        ...
        <div className="bg-white/[0.02] border border-white/[0.04] rounded-2xl p-6">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-lg font-semibold text-white">Top Tenants</h2>
          </div>
          {topTenants.length === 0 ? (
            <p className="text-white/20 text-center py-8 text-sm">No tenant data yet</p>
          ) : (
            <div className="space-y-3">
              {topTenants.map((t, i) => (
                <div key={t._id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-white/[0.02] transition">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                    i === 0 ? 'bg-orange-500 text-white' : 'bg-white/5 text-white/50'
                  }`}>
                    {i + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-white text-sm font-medium truncate">{t.name}</p>
                    <p className="text-white/25 text-[11px] truncate">{t.email}</p>
                  </div>
      ...
      <div className="bg-white/[0.02] border border-white/[0.04] rounded-2xl p-6">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h2 className="text-lg font-semibold text-white">Live Feed — Recent Leads</h2>
          <span className="text-white/20 text-xs ml-auto">Last 20 across all tenants</span>
        </div>
        {recentLeads.length === 0 ? (
          <p className="text-white/20 text-center py-8 text-sm">No leads captured yet across the platform</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/[0.04]">
                  <th className="text-left text-white/30 text-[11px] font-bold uppercase tracking-wider py-3 px-4">Token</th>
                  <th className="text-left text-white/30 text-[11px] font-bold uppercase tracking-wider py-3 px-4">Tenant</th>
                  <th className="text-left text-white/30 text-[11px] font-bold uppercase tracking-wider py-3 px-4">Partner</th>
                  <th className="text-left text-white/30 text-[11px] font-bold uppercase tracking-wider py-3 px-4">Data</th>
                  <th className="text-left text-white/30 text-[11px] font-bold uppercase tracking-wider py-3 px-4">Time</th>
                </tr>
              </thead>
              <tbody>
                {recentLeads.map((lead) => (
                  <tr key={lead._id} className="border-b border-white/[0.02] hover:bg-white/[0.01] transition">
                    ...
                    <td className="py-3 px-4 text-white/25 text-xs">{new Date(lead.submittedAt).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
