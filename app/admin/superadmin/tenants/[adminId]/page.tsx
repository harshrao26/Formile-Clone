    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-foreground/40 mt-1 text-sm">{data.admin.email} · Joined {new Date(data.admin.createdAt).toLocaleDateString()}</p>
          </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          return (
            <div key={tab.key} className="bg-white/[0.02] border border-border rounded-2xl p-5 text-center">
              <Icon className="w-5 h-5 text-orange-400 mx-auto mb-2" />
              <p className="text-2xl font-bold text-foreground">{tab.count}</p>
              <p className="text-foreground/25 text-xs">{tab.label}</p>
            </div>
          );
        })}
      </div>
      ...
      {/* Tabs */}
      <div className="flex gap-1 bg-white/[0.02] p-1 rounded-2xl border border-border w-fit">
        {tabs.map((tab) => (
          <button
            key={tab.key}
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="text-left text-foreground/40 text-[11px] font-bold uppercase py-3 px-4">Token</th>
                      <th className="text-left text-foreground/40 text-[11px] font-bold uppercase py-3 px-4">Partner</th>
                      <th className="text-left text-foreground/40 text-[11px] font-bold uppercase py-3 px-4">Person</th>
                      <th className="text-left text-foreground/40 text-[11px] font-bold uppercase py-3 px-4">Data</th>
                      <th className="text-left text-foreground/40 text-[11px] font-bold uppercase py-3 px-4">Date</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.leads.map((lead) => (
                      <tr key={lead._id} className="border-b border-white/[0.02] hover:bg-white/[0.01]">
                        ...
                        <td className="py-3 px-4 text-foreground/30 text-xs">{new Date(lead.submittedAt).toLocaleDateString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              data.partners.map((p) => (
                <div key={p._id} className="flex items-center justify-between p-4 rounded-xl hover:bg-white/[0.02] transition">
                  <div>
                    ...
                    <p className="text-foreground/25 text-xs">/{p.slug}</p>
                  </div>
            ...
            {data.companies.length === 0 ? (
              ...
              data.companies.map((c) => (
                <div key={c._id} className="flex items-center justify-between p-4 rounded-xl hover:bg-white/[0.02] transition">
                  ...
                  <p className="text-foreground/25 text-xs">{new Date(c.createdAt).toLocaleDateString()}</p>
                </div>
              ))
            )}
          </div>
            ...
            {data.persons.length === 0 ? (
              ...
              data.persons.map((p) => (
                <div key={p._id} className="flex items-center justify-between p-4 rounded-xl hover:bg-white/[0.02] transition">
                  ...
                  <p className="text-foreground/25 text-xs">{new Date(p.createdAt).toLocaleDateString()}</p>
                </div>
              ))
            )}
          </div>
